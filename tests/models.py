...
class BaseForeign(SQLModel):
    name: str = Field(index=True)
    foo_id: int | None = Field(default=None, foreign_key="foo.id")
...
class ManyToManyLinkModel(SQLModel, table=True):  # type: ignore[call-arg]
    many_id: int | None = Field(default=None, primary_key=True, foreign_key="manytomanymodel.id")
    model_id: int | None = Field(default=None, primary_key=True, foreign_key="my_model.id")


class BaseMyModel(BaseModel):
    created_at: datetime | None = Field(None, sa_column=Column(DateTime(timezone=True)))
    another_id: int
    int_number: int
    float_number: int
    is_boolean: bool
...
class MyModelBase(BaseMyModel, SQLModel):
    foreign_id1: int | None = Field(
        default=None,
        foreign_key="foreign.id",
    )
    foreign_id2: int | None = Field(
        default=None,
        foreign_key="foreign.id",
    )
...
class MyModel(MyModelBase, table=True):  # type: ignore[call-arg]
    ...
    parent_id: int | None = Field(default=None, foreign_key="my_model.id")
    ...
    one_to_many: list["OneToManyModel"] | None = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[OneToManyModel.my_model_id]"}
    )
    many_to_many: list[ManyToManyModel] | None = Relationship(link_model=ManyToManyLinkModel)
    if PYDANTIC_V2:
...
class MyModelCreate(MyModelBase):
    foreign1: ForeignCreate | None = None
    foreign2: ForeignCreate | None = None
